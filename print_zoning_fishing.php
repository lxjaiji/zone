...
if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
    $id = trim($_GET["id"]);
    // Fetch clearance details, could also join with users table if 'encoded by' is needed on printout
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                die("Error: No clearance found with ID $id. Cannot generate print view.");
            }
        } else {
...
if ($clearance === null) {
    die("Error: Clearance data could not be loaded. Cannot generate print view.");
}
...
<head>
    <meta charset="UTF-8">
    <title>Print Zoning Fishing Structure/Gear Clearance</title>
    <style>
        body {
...
<body onload="window.print();">
    <div class="print-container">
        ...
        <h1 style="text-align:center;">ZONING FISHING STRUCTURE/GEAR CLEARANCE</h1>

        <p style="text-align:right;">Date Issued: <?php echo formatDate($clearance['date_of_issuance']); ?></p>

        <p>TO WHOM IT MAY CONCERN:</p>
        <p style="text-indent: 2em;">This is to certify that <strong><?php echo htmlspecialchars($clearance['owner']); ?></strong>, a resident of <?php echo htmlspecialchars($clearance['resident_of']); ?>, has been granted a Zoning Clearance for the operation of a <strong><?php echo htmlspecialchars($clearance['fishing_structure_gear']); ?></strong> located at <?php echo htmlspecialchars($clearance['located_in']); ?>.</p>

        <p>This clearance is issued for the purpose of securing necessary permits and is subject to the conditions stipulated in the Zoning Ordinance and other applicable laws, rules, and regulations.</p>

        <p>Paid under O.R. No.: <strong><?php echo htmlspecialchars($clearance['or_number']); ?></strong></p>
        <p>Amount Paid: PHP <strong><?php echo number_format($clearance['amount_paid'], 2); ?></strong></p>
        <p>Date Paid: <?php echo formatDate($clearance['date_paid']); ?></p>
        <p>Issued at: <?php echo htmlspecialchars($clearance['issued_at']); ?></p>
        <p>Encoded By: <strong><?php echo htmlspecialchars($clearance['encoded_by_username'] ?? 'N/A'); ?></strong></p>


        <div class="footer-section">
            <p style="text-align:center;">Issued this <?php echo date("jS", strtotime($clearance['date_of_issuance'])); ?> day of <?php echo date("F, Y", strtotime($clearance['date_of_issuance'])); ?> at <?php echo htmlspecialchars($clearance['issued_at']); ?>.</p>

            <div class="signature-block">
                 <div class="signature-line"></div>
                 <p class="signature-name"><?php echo htmlspecialchars($clearance['signatory']); ?></p>
            </div>
        </div>

        <div class="important-note">
            <p><strong>Note:</strong> This clearance does not exempt the applicant from complying with other requirements from other government agencies. Any alteration or erasure on this certificate renders it null and void.</p>
        </div>
